...
describe('Registration App', () => {
  ...
  it('Should disable add Receipe when MaxLimit', () => {
    ...
    cy.get('[data-name="col-sm-12 col-md-6 col-xl-4"]').eq(1).find('button').should('be.disabled');

    // Add Extra Meal should be disabled
    cy.get('[data-name="col-sm-12 col-md-6 col-xl-4"]').eq(3).find('button').should('be.disabled');

    // total should be
    cy.get('h3').eq(1).contains('$156.82');
  });
